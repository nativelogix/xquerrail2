...
declare private variable $CONTEXT-KEY       := "request:context";
...
declare private variable $CONTEXT-PARAMS    := ($CONTROLLER,$APPLICATION,$ACTION,$FORMAT);
   ...
   let $fields := 
    ...
    let $_ := 
         ...
         then if(xdmp:get-request-method() = ("PUT","POST") and xdmp:get-request-body()) 
              then map:put($request, $BODY, mljson:transform-from-json(xdmp:get-request-body())[1])
              else ()
...
(:~
 :  Returns the parameters as a map and includes the context which a map nested in the context-key
~:)
declare function request:params()  as map:map{
    let $new-map := map:map()
    let $context := map:map()
    let $_ := 
      for $c in $CONTEXT-PARAMS
      return map:put($context,fn:substring-after($c,":"),map:get($request,$c))
    let $_ := 
        (
            ...
            ,
            map:put($new-map,$CONTEXT-KEY,$context)
        )    
    return
        $new-map
};
