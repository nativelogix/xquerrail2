...
declare function xray:generate-suites(
...
{
  let $test-id := xdmp:md5(fn:string(xdmp:random()))
  let $modules := utils:get-modules($test-dir, fn:string($module-pattern))
  let $suites := 
      for $module in $modules
      let $fns := 
        try { utils:get-function-names($module) }
        catch ($ex) { <error>error</error>}
      return
        element module {
          attribute name {
            fn:string-join(fn:tokenize(
               fn:tokenize(utils:relative-path($module),"/")[fn:last()],
            "\.")[1 to fn:last() -1 ],".")
          },
          attribute id {xdmp:md5(utils:relative-path($module))},
          attribute path { utils:relative-path($module) } ,
          if($fns[@name = "setup"]) then attribute setup {fn:true()} else (),
          if($fns[@name = "teardown"]) then attribute teardown {fn:true()} else (),
          for $t in $fns[fn:not(@name = ("setup", "teardown"))]
          return 
             <test>{$t/@*|$t/node()}</test>   
        }
   let $tests := 
